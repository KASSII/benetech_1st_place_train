...
### Dataset
def round_to_specific_digits(num, digits):
    rounded = round(num, digits)
    return rounded

def round_float(value, valid_dicimal):
    if isinstance(value, float):
        str_value = str(value)
        if "e" in str_value:
            str_value = "{:.15f}".format(value)

        if "." in str_value:
            integer, dicimal = str_value.split(".")
            if abs(float(integer)) > 1:
                valid_dicimal = 1
            else:
                valid_dicimal = 4
        value = round_to_specific_digits(value, valid_dicimal)
    return value
...
def gen_data(df):
    ...
    for id, gt_x, gt_y, chart_type, image_path in zip(id_list, gt_x_list, gt_y_list, chart_type_list, image_path_list):
        ...
        valid_dicimal = -1
        all_x = [round_float(x, valid_dicimal) for x in all_x]
        all_y = [round_float(x, valid_dicimal) for x in all_y]
